...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function resolveUrlToMockData(rawUrl, accountsOverride) {
    ...
    if (urlClean.includes('lifenuri.com')) {
      ...
      const groupMatch = rawUrl.match(/themesgroup\/(\d+)/i);
      ...
      if (groupNo && typeof LIFENURI_FALLBACK_DATA !== 'undefined' && LIFENURI_FALLBACK_DATA[groupNo]) {
        console.log("[Lifenuri Scanner] Using cached fallback data for group:", groupNo);
        let plan = "스마트케어5싱글";
        if (accounts === 2) plan = "스마트케어4더블";
        else if (accounts === 3) plan = "스마트케어5트리플";
        else if (accounts === 4) plan = "스마트케어5쿼드";

        const mappedCached = LIFENURI_FALLBACK_DATA[groupNo].map(p => ({
          name: p.name,
          modelName: p.modelName,
          categoryId: p.categoryId,
        ...
        return { data: mappedCached, url: rawUrl };
      }
